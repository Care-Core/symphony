defmodule SymphonyElixir.HoldStoreTest do
  ...
  test "descriptor validation rejects a non-regular open descriptor" do
    workspace_root = workspace_root("descriptor-type")
    state_path = write_json_state!(workspace_root, %{"version" => 1, "holds" => []})
    initial_stat = File.lstat!(state_path)
    {:ok, io_device} = File.open("/dev/null", [:read, :binary])
    on_exit(fn -> File.close(io_device) end)

    assert {:error, {:hold_state_insecure_descriptor, ^state_path, {:invalid_type, type}}} =
             HoldStore.read_validated_descriptor(io_device, state_path, initial_stat)

    refute type == :regular
  end

  test "load rejects an empty state file" do
    workspace_root = workspace_root("empty-state")
    state_path = write_state!(workspace_root, "")

    assert {:error, {:hold_state_invalid, ^state_path, %Jason.DecodeError{}}} =
             HoldStore.load(workspace_root)
  end

  test "descriptor open failure is surfaced after the initial stat" do
    workspace_root = workspace_root("descriptor-open")
    state_path = write_json_state!(workspace_root, %{"version" => 1, "holds" => []})
    initial_stat = File.lstat!(state_path)
    File.rm!(state_path)

    assert {:error, {:hold_state_unreadable, ^state_path, :enoent}} =
  ...
  test "descriptor close returning an unexpected result fails closed" do
    workspace_root = workspace_root("descriptor-close-unexpected")
    state_path = write_json_state!(workspace_root, %{"version" => 1, "holds" => []})
    initial_stat = File.lstat!(state_path)
    ...
    assert {:error, {:hold_state_descriptor_close_failed, ^state_path, {:unexpected_close_result, :unexpected}}} =
             HoldStore.read_open_state_file_for_test(
               state_path,
               initial_stat,
               &HoldStore.read_validated_descriptor/3,
  ...
  test "descriptor close exception fails closed" do
    workspace_root = workspace_root("descriptor-close-raise")
    state_path = write_json_state!(workspace_root, %{"version" => 1, "holds" => []})
    initial_stat = File.lstat!(state_path)

    descriptor_closer = fn io_device ->
      :ok = File.close(io_device)
      raise "simulated descriptor close failure"
    end

    assert {:error, {:hold_state_descriptor_close_failed, ^state_path, {:error, %RuntimeError{message: "simulated descriptor close failure"}}}} =
             HoldStore.read_open_state_file_for_test(
               state_path,
               initial_stat,
               &HoldStore.read_validated_descriptor/3,
  ...
  test "atomic write failure preserves unexpected and raised close outcomes" do
    for {suffix, descriptor_closer, close_reason} <- [
          {"unexpected",
           fn io_device ->
             :ok = File.close(io_device)
             :unexpected
           end, {:unexpected_close_result, :unexpected}},
          {"raised",
           fn io_device ->
             :ok = File.close(io_device)
             raise "simulated atomic close failure"
           end, {:error, %RuntimeError{message: "simulated atomic close failure"}}}
        ] do
      workspace_root = workspace_root("atomic-close-#{suffix}")
      state_path = state_path(workspace_root)
      File.mkdir_p!(workspace_root)
      ...
      combined_reason =
        {:temporary_descriptor_write_and_close_failed, :simulated_write_failure, close_reason}
  ...
  test "atomic descriptor close is attempted without replacing a write exception" do
    workspace_root = workspace_root("atomic-write-raise")
    state_path = state_path(workspace_root)
    File.mkdir_p!(workspace_root)
    test_pid = self()

    descriptor_writer = fn _io_device, _temp_path, _encoded ->
      raise_descriptor_write_error()
    end

    descriptor_closer = fn io_device ->
      send(test_pid, :atomic_descriptor_close_attempted)
      File.close(io_device)
    end

    assert_raise RuntimeError, "simulated descriptor write failure", fn ->
      HoldStore.atomic_private_write_for_test(state_path, "payload",
        descriptor_writer: descriptor_writer,
        descriptor_closer: descriptor_closer
      )
    ...
    assert_received :atomic_descriptor_close_attempted
    refute File.exists?(state_path)
    assert Path.wildcard(state_path <> ".tmp.*") == []
  end

  test "atomic close failure is logged without replacing a write exception" do
    workspace_root = workspace_root("atomic-write-raise-close-failure")
    state_path = state_path(workspace_root)
    File.mkdir_p!(workspace_root)
    ...
    assert log =~ "Hold state temporary descriptor close failed after write raised"
    assert log =~ "simulated_close_failure"
    refute File.exists?(state_path)
    assert Path.wildcard(state_path <> ".tmp.*") == []
  end
  ...
  defp raise_descriptor_write_error, do: raise("simulated descriptor write failure")
end
