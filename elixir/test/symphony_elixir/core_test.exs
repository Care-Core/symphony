defmodule SymphonyElixir.CoreTest do
  ...
  test "config defaults and validation checks" do
    ...
    write_workflow_file!(Workflow.workflow_file_path(),
      tracker_kind: "linear",
      tracker_required_labels: [" Agent Ready ", "AGENT READY", "backend"]
    )

    assert :ok = Config.validate!()
    assert Config.settings!().tracker.required_labels == ["agent ready", "backend"]
  end
  ...
  test "current WORKFLOW.md file is valid and complete" do
    ...
    assert is_nil(Map.get(tracker, "project_slug")) or is_binary(Map.get(tracker, "project_slug"))
  ...
  test "abnormal worker exit increments retry attempt progressively" do
    ...
    assert (due_at_ms - trigger_ms) in 39_500..40_500
  end
